...
// +kubebuilder:rbac:groups=quicksight.services.k8s.aws,resources=analyses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=quicksight.services.k8s.aws,resources=analyses/status,verbs=get;update;patch
...
func (rm *resourceManager) IsSynced(ctx context.Context, res acktypes.AWSResource) (bool, error) {
	...
	if r.ko.Status.CreationStatus == nil {
		return false, nil
	}
	creationStatusCandidates := []string{"CREATION_SUCCESSFUL", "UPDATE_SUCCESSFUL"}
	if !ackutil.InStrings(*r.ko.Status.CreationStatus, creationStatusCandidates) {
		return false, nil
	}
