...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Spec.BrandID = &identifier.NameOrID

	f0, f0ok := identifier.AdditionalKeys["awsAccountID"]
	if f0ok {
		r.ko.Spec.AWSAccountID = aws.String(f0)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	...
	f1, ok := fields["brandID"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: brandID"))
	}
	r.ko.Spec.BrandID = &f1
