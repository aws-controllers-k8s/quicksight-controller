...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Spec.ActionConnectorID = &identifier.NameOrID

	f1, f1ok := identifier.AdditionalKeys["awsAccountID"]
	if f1ok {
		r.ko.Spec.AWSAccountID = aws.String(f1)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	f0, ok := fields["actionConnectorID"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: actionConnectorID"))
	}
	r.ko.Spec.ActionConnectorID = &f0
	f1, ok := fields["awsAccountID"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: awsAccountID"))
	}
	r.ko.Spec.AWSAccountID = &f1
